        ...
        if let Ok(mut current_runtime_lock) = self.runtime.lock() {
            match *current_runtime_lock {
                RunningState::Idle => {
                    ...
                    unsafe {
                        let breakpoint_callback: Box<dyn FnMut(StoppedReasonEnum)> =
                            Box::new(breakpoint_callback);
                        ...
                        let breakpoint_callback: Box<dyn FnMut(StoppedReasonEnum) + Send> =
                            transmute(breakpoint_callback);
